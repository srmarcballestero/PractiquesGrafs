...
//  COMPLETEU-HO CRIDANT LES FUNCIONS DFS_Trees i BFS_Trees EN COMPTES DE DFS I BFS
...
//  Read graph from graph0.in to adjacencies lists G0
  ...
  //  Read graph from graph1.in to adjacencies lists G1
    ...
    Dijkstra(G1, 0, fout);
    fout.close();
//  COMPLETEU-HO AMB graph2.in graph3.in
...
//  Knight 6x6 graph Kt6_6: distances from (2,0)
    ...
    fout.close();
//  COMPLETEU-HO AMB graph_rook i graph_bishop


//  Random weighted complete graph WK10 with weights 1-9 to weighted adjacencies lists 
	fout.open( "WK10.out" );    
    wgraph WK10 = wgraph_complete( 10, 9 );
	wgraph_write( WK10, fout );
    fout << "Distància entre el primer vèrtex i el darrer: ";
    fout << Dijkstra(WK10, 0, 9) << endl;
    fout << "Distància màxima des del primer vèrtex: ";
    fout << Dijkstra(WK10, 0) << endl;
    fout << "Diàmetre:";
    fout << Diameter(WK10) << endl;

    //fout << KruskalTrees(WK10, fout) << " és el pes total dels arbres minimals per Kruskal" << endl;
    fout << PrimTrees(WK10, fout) << " és el pes total dels arbres minimals per Prim" << endl;
...
//  COMPLETEU-HO AMB TOTS ELS ALTRES GRAFS PONDERATS 
//  I AMB TOTES LES INFORMACIONS DEMANADES CRIDANT LES FUNCIONS DEL MÒDUL wgraphMP.cpp
